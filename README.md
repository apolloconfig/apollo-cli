...
`apollo env list`, `apollo namespace list`, `apollo namespace get`, `apollo config get`,
`apollo config list`, `apollo config diff`, and `apollo config apply` read env, namespace, or
configuration item data whose scope can be narrower than the app. Use `user-token` mode for these
commands. Legacy consumer-token mode cannot safely verify env/namespace-level read scope from the
available `/openapi/v1/apps/authorized` response, so the CLI rejects these scoped-read workflows
